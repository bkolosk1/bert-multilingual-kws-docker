...
class Dictionary(object):
    def __init__(self, max_vocab_size = 0):
...
class Corpus(object):
    def __init__(self, df_train, df_valid, df_test, args, unk=True):
        ...
        self.tokenize_df_optim(df_test)
    ...
    def tokenize_doc_driver(self, args):
        ...
        for idx, line in tqdm(df.iterrows(), desc=f"Prepare docs for {name}",total=len(df)):            
            ...
            lang = line['lang']
            stemmer = lang2stemmer.get(lang, PorterStemmer().stem)
...
def get_batch_docs(data, targets, langs, i, cuda):
    if cuda:
        return data[i, :, :].cuda(), targets[i, :, :].cuda() , langs[i].cuda()
    return data[i, :, :], targets[i, :, :], langs[i]
    #return data[i, :, :].cuda(), targets[i, :, :].cuda(), langs[i].cuda()
