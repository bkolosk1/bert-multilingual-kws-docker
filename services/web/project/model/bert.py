...
class BertClassifier(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.config = config
        self.bert = BertModel.from_pretrained('bert-base-multilingual-uncased')#,return_dict=False)
    ...
    def forward(self, input_ids, input_pos=None, lm_labels=None, masked_idx=None, test=False,
        ...
        mask[mask > 0] = 1
        mask = mask.type(torch.uint8)
        hidden_states , _ = self.bert(input_ids=input_ids, attention_mask=mask,return_dict=False)
